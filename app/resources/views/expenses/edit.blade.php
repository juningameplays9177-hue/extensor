@extends('layouts.app', ['title' => 'Editar Conta a Pagar - Top Rio'])

@section('content')
    <h1 class="title">Editar Conta a Pagar</h1>
    <p class="subtitle">Atualize as informacoes da despesa</p>
        ...
        <form method="POST" action="{{ route('expenses.update', $expense) }}">
            ...
            <div class="form-grid">
                ...
                <div>
                    ...
                    <input type="number" id="value" name="value" step="0.01" min="0" value="{{ old('value', $expense->value) }}" required>
                </div>
                <div>
                    <label for="due_date">Data de Vencimento <span style="color: #ef4444;">*</span></label>
                    <input type="date" id="due_date" name="due_date" value="{{ old('due_date', $expense->due_date->format('Y-m-d')) }}" required>
                </div>
                <div>
                ...
                <div id="payment_date_field">
                    ...
                    <input type="date" id="payment_date" name="payment_date" value="{{ old('payment_date', $expense->payment_date ? $expense->payment_date->format('Y-m-d') : '') }}">
                </div>
                <div style="grid-column: 1 / -1;">
                    <label for="notes">Observacoes</label>
                    <textarea id="notes" name="notes" rows="3">{{ old('notes', $expense->notes) }}</textarea>
                </div>
            </div>
